...
void MQTT::ZIGBEE_write() {
   ...
   // hex[5] = 0x00;  // short address (2 bytes, will be set later)
   // hex[6] = 0x00;  // short address (2 bytes, will be set later)
    ...
    //hex[payloadLength + 16] = '}'; // Add the closing curly brace for the JSON object
    //hex[payloadLength + 17] = '\0'; // Null-terminate the string
...
void MQTT::begin() {
    ...
    client.setKeepAlive(60);                 // Set keep-alive interval (e.g., 60 seconds)
    
    client.setCallback(MQTT::Subscribe); // Set callback to static function
    int i =0;
