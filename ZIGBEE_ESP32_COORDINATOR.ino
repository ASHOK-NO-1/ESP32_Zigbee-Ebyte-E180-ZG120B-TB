...
void Save_Short_Address_Mapping(const ShortAddressMapping& mapping) {
    ...
    if (!shortAddressExists) {
        ...
        String FloorNoHex = "0x" + String("0" + String(mapping.FloorNo, HEX)).substring(max(0, static_cast<int>(2 - String(mapping.FloorNo, HEX).length())));
        String RoomNoHex = "0x" + String("0" + String(mapping.RoomNo, HEX)).substring(max(0, static_cast<int>(2 - String(mapping.RoomNo, HEX).length())));
        String SensorIDHex = "0x" + String("0" + String(mapping.SensorID, HEX)).substring(max(0, static_cast<int>(2 - String(mapping.SensorID, HEX).length())));
        String SensorNoHex = "0x" + String("0" + String(mapping.SensorNo, HEX)).substring(max(0, static_cast<int>(2 - String(mapping.SensorNo, HEX).length())));
        
       
        JsonObject mappingObj = sensorAddresses["mappings"].createNestedObject();
        mappingObj["FloorNo"] = FloorNoHex;
        mappingObj["RoomNo"] = RoomNoHex;
        mappingObj["SensorID"] = SensorIDHex;
        mappingObj["SensorNo"] = SensorNoHex;
